...
| INF-02 | Postgres setup: job/state tables, JSONB graph store, pgvector extension | Infra | Must | Sprint 1 | E1 | Done | Schema migrations run cleanly; job, document, chunk, and graph tables exist; pgvector extension enabled. **Correction (2026-07-19):** the compose DB image was `postgres:16-alpine` (no `vector` extension) — pgvector was NOT actually enabled. Fixed with OUT-04: image → `pgvector/pgvector:pg16`, `CREATE EXTENSION vector` in `run_pgvector()`. See `docs/DECISION-vector-retrieval-pgvector.md` | INF-01 |
...
| ING-06-BUG | **OCR "low scan quality" false positive** — native-text PDFs with a few blank/furniture pages wrongly trigger the doc-level warning; blank pages OCR at ~0% and drag the mean below threshold. Fix: skip blank pages, exclude empty-OCR from the mean, native-text gate. Tracked in `docs/ISSUE-ocr-quality-false-positive.md` (+ `.ctx.md`); tasks OCRQ-01..06 | Ingestion | Should | — | E2 | In Progress | Per `ISSUE-ocr-quality-false-positive.md` §5: DDIA job no longer flagged; blank pages excluded from mean; genuine scans still warn (no regression). **OCRQ-01..05 done** (blank-page exclusion + native-text gate `OCR_DOC_MIN_RATIO`, `_summarize_ocr_quality`, tests pass; UI strong-vs-soft banner branch); OCRQ-06 (backfill of pre-fix docs) remains | ING-06 |
...
| OUT-04 | Embedding index: embed all chunks using the chosen embedding model; store in pgvector with `chunk_id → chapter_id` mapping | Q&A | Must | Sprint 4 | E2 | Done | All chunks for a processed book are indexed in pgvector; nearest-neighbor search returns top-k chunks with chapter references in < 500 ms for a 300-page book. **Correction (2026-07-19):** originally marked Done but never wired — vectors lived in `graph` JSON + Python cosine (RC-14). Now actually implemented per `docs/DECISION-vector-retrieval-pgvector.md`: DB image → `pgvector/pgvector:pg16`, `chunk_embeddings` table + `<=>` KNN in `app/embeddings_store.py`, write path in `tasks.py`, read path in `chat.py`/`chat_routes.py` (graph grounding preserved). Embeddings wired end-to-end (`nvidia/nv-embedqa-e5-v5`, 1024-dim). **OUT-04f (backfill) + OUT-04e (sub-chunk embedding) done**: all 24 jobs indexed → 22,546 vectors (chunk sub-passages ~1,341→13,262 for full-coverage retrieval); embedding centralized in `tasks` so graph JSON no longer stores vectors (OUT-04c de-bloat done too). Remaining follow-ups: ANN index/pinned dim, cross-doc/library chat (DECISION §7) | ING-08, INF-02, SK-04 |
