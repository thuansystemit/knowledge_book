...
| RC-13 | Synonym expansion + stemming on the query and concept text (lexical recall for paraphrased questions) | Retrieval | Could | E2 | Done | "drawbacks" matches a concept named "limitations"; measured lift on RC-16. **Done:** stemming (morphological 0.00→1.00) + curated technical-synonym lexicon `_SYNONYMS`/`_expand` (synonym-paraphrase **0.33→1.00**) + expanded stop-words for precision (open-domain paraphrase correctly 0.00 → RC-14 headroom). Core recall & off-topic honesty held at 1.00; regression-tested. **Note:** open-domain synonymy needs embeddings (RC-14) | RC-01 |
| RC-14 | Local semantic retrieval via an **embedding** index (no generative LLM, no per-token billing) — the one "gray area" vs. strict no-model | Retrieval | Could | E2 | Done | Paraphrased queries retrieve the right chunks; runs locally; clearly labelled as embeddings, not chat-LLM. **Done:** `app/embeddings.py` (ollama/openai backends + cosine/rank), config-gated by `EMBEDDING_MODEL` (OFF by default), fail-safe; pipeline embeds concepts+chunks into the graph (preserved through retry); `compose_answer(query_vector=…)` augments lexical hits semantically. Pure math + augmentation unit-tested 9/9 (incl. open-paraphrase rescue); default-off path verified no-regression. **Live semantic needs an embedding model configured + docs re-processed.** Storage note: vectors live in the graph JSON; pgvector (OUT-04) is the scale path | OUT-04 (embedding index) |
...
| RC-17 | Conversational follow-up without an LLM: cheap query rewriting (carry the last concept/topic into the next question) | Retrieval | Could | E2 | Done | "and its drawbacks?" resolves against the previous turn's concept. **Done:** `_is_followup` detects thin anaphoric turns (≤1 content token + a pronoun); `compose_answer(prev_question=...)` merges the prior turn's topic; `chat_routes` passes the previous user message. Unit-tested (7/7) + live-verified two-turn ("tell me more about it" → prior topic) | RC-01 |
...
| ID | Feature / Task | Category | Priority | Owner | Status | Acceptance Criteria | Dependencies |
|---|---|---|---|---|---|---|---|
| RC-20 | Value-ladder rework: **Free = retrieval chat**, **Pro = LLM-synthesized chat** (instead of withholding Q&A from Free entirely) | Paywall | Should | Product | Done | Free users can ask grounded retrieval questions; LLM mode is gated to Pro/Scholar; decision recorded and PAY-01 updated. **Done:** `effective_chat_mode(user,cfg)` — server LLM mode gated by plan via `CHAT_LLM_MIN_PLAN` (default pro; admins bypass; on-prem sets "free"); wired into `ask`, `stream_answer`, and `/api/models` (per-user `chat_mode`); frontend already adapts (hides model picker + shows "no AI" badge in retrieval). Billing copy updated (Free = "Grounded Q&A (no AI)", Pro = "AI-generated Q&A"). Gating unit-tested 8/8 | PAY-01, RC-08 |
...
| RC-22 | Contextual upgrade prompt: offer Pro's LLM chat when a retrieval answer is low-confidence (RC-15) | Paywall | Could | E1 | Done | A weak retrieval answer surfaces a "get a fuller answer with Pro" prompt. **Done:** `compose_answer` returns a `weak` flag; `chat_routes` sends `upgrade` in the SSE done frame only when weak **and** upgrading would unlock LLM (server LLM mode + plan-gated non-admin user); `ChatTab` renders an "Upgrade to Pro" banner. Weak-flag unit-tested (4/4); gating reuses RC-20 | RC-15, RC-20, PAY-06 |
