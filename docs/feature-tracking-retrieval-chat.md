...
| ID | Feature / Task | Category | Priority | Owner | Status | Acceptance Criteria | Dependencies |
|---|---|---|---|---|---|---|---|
| RC-11 | BM25 / TF-IDF ranking over section chunks (in addition to concept-node overlap) | Retrieval | Should | E2 | Proposed | Measurably better recall than keyword overlap on an eval set (RC-16); pure Python, no model | RC-10, RC-16 |
| RC-12 | Extractive answers: return the top matching source **excerpts verbatim** as the answer body, alongside concept definitions | Compose | Should | E2 | Proposed | Answers to passage-style questions include the relevant verbatim excerpt + citation | RC-10 |
| RC-13 | Synonym expansion + stemming on the query and concept text (lexical recall for paraphrased questions) | Retrieval | Could | E2 | Proposed | "drawbacks" matches a concept named "limitations"; measured lift on RC-16 | RC-01 |
...
| RC-15 | Retrieval confidence + honest "not covered": when top score is below a threshold, say the document doesn't cover it rather than forcing a weak answer | Trust | Should | E2 | Proposed | Low-confidence questions return "not covered in this document"; threshold documented/tunable | RC-01 |
| RC-16 | Retrieval-quality eval harness: a fixed Q→expected-concept set to score recall/precision of RC-01/11/13 changes | QA | Should | E2 | Proposed | A script reports recall@k on the eval set; regressions caught before merge | RC-01 |
...
| RC-21 | Answer provenance badge in the UI: label retrieval answers "Answered from the document — no AI generation" vs. LLM answers | UX | Should | E1 | Proposed | The chat UI shows which engine answered (reads `chat_mode` / message `model`) | RC-07, RC-08 |
